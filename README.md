...
- **Buy block order (MAN, new format, 15 Sep):** product name (no brand line) → price → NDD countdown → Size (+ recommender + model line) → Body Fit → Colour → product tabs (solid pills, on by default, keep their scroll position) → set entry point → buy buttons. Live mode still mirrors production.
